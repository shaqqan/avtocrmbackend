...
export class ValidationErrorHandler extends ValidationPipe {
    ...
    await Promise.all(
      validationErrors.map(async (error) => {
        ...
        if (error.constraints) {
          const messages = await this.translateConstraints(
            error.constraints,
            error.property,
            error.value,
          );
        ...
        if (error.children && error.children.length > 0) {
          const nestedErrors = await this.extractErrors(
            error.children,
            propertyPath,
          );
  ...
  private async translateConstraints(
    constraints: Record<string, string>,
    property: string,
    value: any,
  ): Promise<string[]> {
    const translationPromises = Object.values(constraints).map(
      async (message) => {
        try {

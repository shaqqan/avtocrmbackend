...
export class BookAudiobookLinkService {
    ...
    async create(createLinkDto: CreateBookAudiobookLinkDto): Promise<MessageWithDataResponseDto<BookAudiobookLinkResponseDto>> {
        ...
        const book = await this.bookRepository.findOne({ where: { id: createLinkDto.bookId } });
        if (!book) {
        ...
        const audiobook = await this.audiobookRepository.findOne({ where: { id: createLinkDto.audiobookId } });
        if (!audiobook) {
            throw new NotFoundException(
        ...
        const linkEntity = BookAudiobookLinkMapper.toEntityFromCreateDto(createLinkDto);
    ...
    async findAll(query: QueryBookAudiobookLinkDto): Promise<BasePaginationResponseDto<BookAudiobookLinkResponseDto>> {
        const {
            take = 10,
            skip = 0,
            page = 1,
    ...
    async update(id: number, updateLinkDto: UpdateBookAudiobookLinkDto): Promise<MessageWithDataResponseDto<BookAudiobookLinkResponseDto>> {
        ...
        if (updateLinkDto.bookId || updateLinkDto.audiobookId) {
            ...
            const newAudiobookId = updateLinkDto.audiobookId || existingLink.audiobookId;
            ...
            if (updateLinkDto.bookId) {
                const book = await this.bookRepository.findOne({ where: { id: updateLinkDto.bookId } });
                if (!book) {
                    throw new NotFoundException(
                        await this.i18n.translate('errors.BOOK.NOT_FOUND')
                    );
            ...
            if (updateLinkDto.audiobookId) {
                const audiobook = await this.audiobookRepository.findOne({ where: { id: updateLinkDto.audiobookId } });
                if (!audiobook) {
                    throw new NotFoundException(
                        await this.i18n.translate('errors.AUDIOBOOK.NOT_FOUND')
                    );
        ...
        const updatedEntity = BookAudiobookLinkMapper.toEntityFromUpdateDto(updateLinkDto, existingLink);
    ...
    async findLinksByBookId(bookId: number): Promise<BookAudiobookLinkSummaryDto[]> {
    ...
    async findLinksByAudiobookId(audiobookId: number): Promise<BookAudiobookLinkSummaryDto[]> {
        const links = await this.linkRepository.find({
            where: { audiobookId },
            relations: ['book'],
            order: { priority: 'ASC', createdAt: 'DESC' },

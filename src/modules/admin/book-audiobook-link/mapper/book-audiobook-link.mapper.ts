import { BookAudiobookLink, LinkStatusEnum, LinkTypeEnum } from '../../../../databases/typeorm/entities/book-audiobook-link.entity';
...
export class BookAudiobookLinkMapper {
    ...
    static toSummaryDto(entity: BookAudiobookLink, locale: 'uz' | 'ru' | 'en' = 'uz'): BookAudiobookLinkSummaryDto {
    ...
    static toDtoList(entities: BookAudiobookLink[]): BookAudiobookLinkResponseDto[] {
        return entities.map(entity => this.toDto(entity));
    }

    static toSummaryDtoList(entities: BookAudiobookLink[], locale: 'uz' | 'ru' | 'en' = 'uz'): BookAudiobookLinkSummaryDto[] {
        return entities.map(entity => this.toSummaryDto(entity, locale));
    }

    static toEntityFromCreateDto(dto: CreateBookAudiobookLinkDto): BookAudiobookLink {
    ...
    static toEntityFromUpdateDto(dto: UpdateBookAudiobookLinkDto, existingLink: BookAudiobookLink): BookAudiobookLink {
        if (dto.bookId !== undefined) existingLink.bookId = dto.bookId;
        if (dto.audiobookId !== undefined) existingLink.audiobookId = dto.audiobookId;
        if (dto.linkType !== undefined) existingLink.linkType = dto.linkType;
        if (dto.status !== undefined) existingLink.status = dto.status;
        if (dto.description !== undefined) existingLink.description = dto.description;

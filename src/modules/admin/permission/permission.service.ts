...
export class PermissionService {
  ...
  async create(
    ...
    const permission =
      PermissionMapper.toEntityFromCreateDto(createPermissionDto);
    const savedPermission = await this.permissionRepository.save(permission);
    return new MessageWithDataResponseDto(
      this.i18n.t('messages.PERMISSION.CREATED'),
      PermissionMapper.toDto(savedPermission),
    );
  }
  ...
  async update(
    ...
    const updatedPermission = PermissionMapper.toEntityFromUpdateDto(
      updatePermissionDto,
      permission,
    );
    ...
    return new MessageWithDataResponseDto(
      this.i18n.t('messages.PERMISSION.UPDATED'),
      PermissionMapper.toDto(savedPermission),
    );
  }

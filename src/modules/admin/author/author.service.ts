...
export class AuthorService {
  ...
  async create(createAuthorDto: CreateAuthorDto) {
    const author = await this.authorRepository.save(
      AuthorMapper.toEntityFromCreateDto(createAuthorDto),
    );
  ...
  async findAll(query: BasePaginationDto) {
    ...
    if (sortBy === 'name') {
      sortColumn = 'name_' + locale;
    } else if (sortBy === 'lastName') {
      sortColumn = 'lastName_' + locale;
    } else if (sortBy === 'middleName') {
      sortColumn = 'middleName_' + locale;
    } else if (sortBy === 'description') {
      sortColumn = 'description_' + locale;
    }
    ...
    return new BasePaginationResponseDto(AuthorMapper.toDtoList(authors), {
      total,
      page,
      limit,
    });
  ...
  public async list() {
    ...
    const authors = await this.authorRepository.find({
      select: {
        id: true,
        [`name_${locale}`]: true,
        [`lastName_${locale}`]: true,
        [`middleName_${locale}`]: true,
      },
    });

...
export class FeedbacksThemeService {
  ...
  public async findAll(query: BasePaginationDto) {
    ...
    const [feedbacksThemes, total] =
      await this.feedbacksThemeRepository.findAndCount({
        select: {
          id: true,
          [`name_${locale}`]: true,
          createdAt: true,
  ...
  public async findOne(id: number) {
    ...
    if (!feedbacksTheme) {
      throw new NotFoundException(
        this.i18n.t('errors.FEEDBACKS_THEME.NOT_FOUND'),
      );
    }
  ...
  async update(id: number, updateFeedbacksThemeDto: UpdateFeedbacksThemeDto) {
    ...
    const updatedTheme = await this.feedbacksThemeRepository.save({
      ...feedbacksTheme,
      ...FeedbacksThemeMapper.toEntityFromUpdateDto(
        updateFeedbacksThemeDto,
        feedbacksTheme,
      ),
    });
  ...
  async remove(id: number) {
    ...
    return new MessageResponseDto(
      this.i18n.t('success.FEEDBACKS_THEME.DELETED'),
    );
  }
}

...
export class UploadController {
  ...
  @ApiOperation({
    ...
    description:
      'Uploads and processes a single file with comprehensive validation and metadata extraction. Supports multiple file types and categories with automatic format detection.',
  })
  ...
  async uploadFile(@Req() req: FastifyRequest & { body: any }) {
    ...
    this.logger.log(
      `Upload request: ${uploadData.category}/${uploadData.format}`,
    );
    ...
    this.logger.log(
      `Upload completed in ${processingTime}ms: ${result.data.id}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Processes multiple files concurrently with controlled concurrency and comprehensive error handling. Returns detailed results for each file.',
  })
  @ApiConsumes('multipart/form-data')
  @ApiBody({
  ...
  @ApiBadRequestResponse({
    description:
      'Invalid files, too many files, or missing required parameters',
  })
  async uploadBatch(
    ...
    this.logger.log(
      `Batch upload request: ${uploadOptions.category}/${uploadOptions.format}, max: ${uploadOptions.maxFiles}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves comprehensive statistics about uploaded files including storage usage, file distributions, and performance metrics.',
  })
  @ApiOkResponse({
  ...
  async getUploadStats(@Query() query: UploadStatsQueryDto) {
    this.logger.log(
      `Stats request: category=${query.category}, dateFrom=${query.dateFrom}, dateTo=${query.dateTo}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Permanently deletes a file and its metadata from both storage and database with transactional safety.',
  })
  @ApiParam({
    name: 'id',
    type: Number,
  ...
  @ApiOperation({
    ...
    description:
      'Removes expired files from storage and database to free up space. This is a maintenance operation.',
  })
  @ApiOkResponse({
  ...
  @ApiOperation({
    ...
    description:
      'Checks the health status of upload service components including storage and database connectivity.',
  })
  @ApiOkResponse({
  ...
  @ApiOperation({
    ...
    description:
      'Returns current upload configuration including limits, allowed file types, and supported categories.',
  })
  @ApiOkResponse({

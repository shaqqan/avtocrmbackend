...
export class NewsService {
  ...
  async create(createNewsDto: CreateNewsDto) {
    const news = await this.newsRepository.save(
      NewsMapper.toEntityFromCreateDto(createNewsDto),
    );
    return new MessageWithDataResponseDto(
      this.i18n.t('success.NEWS.CREATED'),
      NewsMapper.toDto(news),
    );
  }
  public async findAll(query: BasePaginationDto) {
    ...
    return new BasePaginationResponseDto(NewsMapper.toDtoList(news), {
      total,
      page,
      limit,
    });
  ...
  async update(id: number, updateNewsDto: UpdateNewsDto) {
    ...
    return new MessageWithDataResponseDto(
      this.i18n.t('success.NEWS.UPDATED'),
      NewsMapper.toDto(updatedNews),
    );
  }

...
export class ReviewsAudiobookService {
  ...
  async create(createReviewsAudiobookDto: CreateReviewsAudiobookDto) {
    ...
    if (!audiobook) {
      throw new BadRequestException(
        this.i18n.t('errors.REVIEWS_AUDIOBOOK.AUDIOBOOK_NOT_FOUND'),
      );
    }
    ...
    if (createReviewsAudiobookDto.userId) {
      ...
      if (!user) {
        throw new BadRequestException(
          this.i18n.t('errors.REVIEWS_AUDIOBOOK.USER_NOT_FOUND'),
        );
      }
    }
    ...
    if (existingReview) {
      throw new BadRequestException(
        this.i18n.t('errors.REVIEWS_AUDIOBOOK.ALREADY_REVIEWED'),
      );
    }
  ...
  public async findAll(query: BasePaginationDto) {
    ...
    return new BasePaginationResponseDto(
      ReviewsAudiobookMapper.toDtoList(data),
      {
        total,
        page,
        limit,
  ...
  public async findOne(id: number) {
    ...
    if (!reviewsAudiobook) {
      throw new NotFoundException(
        this.i18n.t('errors.REVIEWS_AUDIOBOOK.NOT_FOUND'),
      );
    }
  ...
  async update(
    id: number,
    updateReviewsAudiobookDto: UpdateReviewsAudiobookDto,
  ) {
    const reviewsAudiobook = await this.reviewsAudiobookRepository.findOne({
      where: { id },
      relations: { user: true },
    ...
    if (
      updateReviewsAudiobookDto.audiobooksId &&
      updateReviewsAudiobookDto.audiobooksId !== reviewsAudiobook.audiobooksId
    ) {
      const audiobook = await this.audioBookRepository.findOne({
    ...
    if (
      updateReviewsAudiobookDto.userId !== undefined &&
      updateReviewsAudiobookDto.userId !== reviewsAudiobook.userId
    ) {
    ...
    const updatedReview = await this.reviewsAudiobookRepository.save({
      ...reviewsAudiobook,
      ...ReviewsAudiobookMapper.toEntityFromUpdateDto(
        updateReviewsAudiobookDto,
        reviewsAudiobook,
      ),
    });
  ...
  async remove(id: number) {
    ...
    return new MessageResponseDto(
      this.i18n.t('success.REVIEWS_AUDIOBOOK.DELETED'),
    );
  }
}

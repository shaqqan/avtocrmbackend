...
export class OptimizeRatingQueries1710000000001 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX idx_review_books_rating ON review_books`,
    );
    await queryRunner.query(
      `DROP INDEX idx_review_books_book_id_rating ON review_books`,
    );
    await queryRunner.query(
      `DROP INDEX idx_reviews_audiobooks_rating ON reviews_audiobooks`,
    );
    await queryRunner.query(
      `DROP INDEX idx_reviews_audiobooks_audiobooks_id_rating ON reviews_audiobooks`,
    );
    await queryRunner.query(
      `DROP INDEX idx_books_published_deleted_rating ON books`,
    );
    await queryRunner.query(
      `DROP INDEX idx_audiobooks_published_rating ON audiobooks`,
    );
    ...
    await queryRunner.query(
      `DROP INDEX idx_books_authors_book_id_rating ON books_authors_authors`,
    );
    await queryRunner.query(
      `DROP INDEX idx_audiobooks_authors_audiobook_id_rating ON audiobooks_authors_authors`,
    );
  }
}
